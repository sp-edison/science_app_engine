...
//		}
//		command = "module list";
		command = ScienceAppConstants.MODULE_CMD+" avail";
...
System.out.println(line);
				...
				if (lineCnt % 2 == 1) {
					...
					while (st.hasMoreTokens()) {
						...
						mod.setIsLoaded(false); // not yet loaded.
System.out.println(modName+"|"+modPath+"|"+mod.getIsLoaded());
						try{
...
//			p1.waitFor();
		...
		return ScienceAppConstants.SUCCESS;
	}
	...
	public List<CommonModule> retrieveAllModules() {
		...
		if(resList.size() == 0){
			...
			// update module data
			String errMsg = updateModules();
			if(errMsg.contains(ScienceAppConstants.NO_MODULE_UPLOAD)){
				return resList;
			}else if(errMsg.contains(ScienceAppConstants.AUTH_FAILURE)){
				return null;
			}
